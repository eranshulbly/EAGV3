...
async def enumerate_interactives(page: Page) -> PageSnapshot:
    # Robustness (S9 assignment extension): on sites whose filters/sorts are
    # plain <a> links (GitHub Trending, many listing pages), the previous
    # turn's click triggers a full navigation. If we evaluate while that
    # navigation is in flight Playwright raises "Execution context was
    # destroyed". Wait for the new document to settle and retry once before
    # giving up — this keeps the a11y loop alive across link-driven page
    # transitions without changing any cascade/driver logic.
    try:
