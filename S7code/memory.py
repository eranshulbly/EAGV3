...
def add_fact(
    ...
    *,
    ...
    classifier (kind is known). When `value['chunk']` holds the underlying
    text we embed that instead of the descriptor — the descriptor is just a
    path + 120-char preview, so embedding it alone leaves most of each
    chunk's semantic content unsearchable."""
    chunk_text = (value or {}).get("chunk")
    embed_input = chunk_text if isinstance(chunk_text, str) and chunk_text.strip() else descriptor
    embedding = _try_embed(embed_input, task_type="retrieval_document")
    item = MemoryItem(
        id=new_id("mem"),
        kind="fact",
