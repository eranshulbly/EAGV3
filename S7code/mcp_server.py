...
index_document:    Chunks a sandbox text file (.md/.txt) or artifact and writes
                   the chunks as fact records into Memory.
index_pdf:         Same as index_document but for .pdf files; preserves page
                   provenance on each chunk via pypdf.
...
def index_document(path: str, chunk_size: int = 400, overlap: int = 80) -> dict:
    """Chunk a sandbox text file (`.md`, `.txt`, or similar) or artifact and write each chunk into Memory as a searchable `fact`. Use this when the content must remain retrievable across later turns or runs (an indexing step before later vector queries). For one-shot inspection of a known file's contents in this turn, prefer `read_file` instead. For `.pdf` files, use `index_pdf` — `index_document` will treat the binary as text and produce garbage chunks. Example: index_document("notes/spec.md")."""
...
def _extract_pdf_pages(path: str) -> list[str]:
    """Return one string per PDF page, in order. Sandbox-relative paths only."""
...
def _chunks_with_pages(pages: list[str], size: int, overlap: int) -> list[tuple[str, int, int]]:
    """Chunk concatenated page text while tracking which pages each chunk spans.

    Returns a list of (chunk_text, page_start, page_end) triples with pages
    numbered from 1. Word boundaries are tracked per page so a chunk that
    crosses a page break gets a range like (3, 4)."""
    page_word_counts = [len(p.split()) for p in pages]
    # cumulative[i] = total words across pages[0..i-1]; cumulative[-1] = total
    cumulative = [0]
    ...
    def page_for_word_index(idx: int) -> int:
        # Return 1-based page number containing the word at position idx.
        for page_idx in range(len(pages)):
            if cumulative[page_idx] <= idx < cumulative[page_idx + 1]:
                return page_idx + 1
        return len(pages)  # idx past the end clamps to last page

    out: list[tuple[str, int, int]] = []
    stride = max(1, size - overlap)
    i = 0
    while i < len(all_words):
        end = min(i + size, len(all_words))
        text = " ".join(all_words[i:end])
        out.append((text, page_for_word_index(i), page_for_word_index(end - 1)))
...
@mcp.tool()
def index_pdf(path: str, chunk_size: int = 400, overlap: int = 80) -> dict:
    """Chunk a sandbox `.pdf` file and write each chunk into Memory as a searchable `fact`. Preserves page provenance: each chunk records the page range it spans, surfaced both in its descriptor and in its metadata. Use this for any `.pdf` content that must remain retrievable across later turns or runs. For `.md` / `.txt` files use `index_document` instead — this tool runs `pypdf` text extraction and assumes a real PDF. Example: index_pdf("travel/paris.pdf")."""
    if not path.lower().endswith(".pdf"):
        raise ValueError(f"index_pdf expects a .pdf path; got {path!r}. Use index_document for text files.")
    ...
    if not any(p.strip() for p in pages):
        return {
            ...
            "chunks_indexed": 0,
            "warning": "no extractable text (scanned PDF? OCR not supported here)",
        }
    triples = _chunks_with_pages(pages, size=chunk_size, overlap=overlap)
    source = f"sandbox:{path}"
    run_id = f"index-{datetime.now().strftime('%Y%m%d%H%M%S')}"
    indexed = 0
    for i, (chunk, p_start, p_end) in enumerate(triples):
        page_label = f"p.{p_start}" if p_start == p_end else f"pp.{p_start}-{p_end}"
        preview = chunk[:120].replace("\n", " ")
        descriptor = f"[{source} {page_label}, chunk {i+1}/{len(triples)}] {preview}"
        _memory.add_fact(
            descriptor=descriptor,
            value={
                "chunk": chunk,
