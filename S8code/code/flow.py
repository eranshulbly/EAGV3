...
class Graph:
    ...
    def extend_from(self, src_nid: str, result: AgentResult,
        ...
        # Static internal_successors (Coder → SandboxExecutor). Splice the
        # executor ONTO the edge rather than hanging it off `src` in
        # parallel: any node already consuming `src`'s output is re-pointed
        # to depend on the executor instead. Without this the executor and
        # the real consumer (e.g. a Formatter the Planner wired to the
        # Coder) become siblings — the Formatter would answer from the raw
        # code text while the EXECUTED result (sandbox stdout) goes to a
        # dead-end leaf. Mirrors the Critic auto-insertion splice below.
        if src_def.internal_successors:
            # Capture consumers wired to `src` BEFORE adding the executor,
            # so we don't reroute the executor through itself.
            prior_children = list(self.g.successors(src_nid))
            exec_nids: list[str] = []
            for child_skill in src_def.internal_successors:
                nid = self.add_node(child_skill, inputs=[src_nid])
                added.append(nid)
                exec_nids.append(nid)
            for child_nid in prior_children:
                self.g.remove_edge(src_nid, child_nid)
                # Rewrite the child's input handles: a reference to `src`
                # now points at the executor(s) so the input renderer pulls
                # the sandbox result instead of the code text.
                new_inputs = [i for i in self.g.nodes[child_nid]["inputs"]
                              if i != src_nid]
                for exec_nid in exec_nids:
                    self.g.add_edge(exec_nid, child_nid)
                    new_inputs.append(exec_nid)
...
def main() -> None:
    ...
    if args and args[0] == "--resume":
        # Resume reads the query back from query.txt on disk; no prompt needed.
        resume_sid = args[1] if len(args) > 1 else None
        query = " ".join(args[2:])
    else:
        # Interactive: ask the user for the query after the app starts, rather
        # than taking it from the run command. Any words passed on the command
        # line are still honoured as a default so scripted runs keep working.
        query = " ".join(args)
        if not query:
            print("Enter your query (multi-line OK). Finish with an empty "
                  "line, or Ctrl-D:")
            lines: list[str] = []
            while True:
                try:
                    line = input()
                except EOFError:        # Ctrl-D / end of piped stdin
                    break
                except KeyboardInterrupt:
                    print("\n[flow] cancelled; exiting.")
                    return
                if line == "" and lines:  # blank line ends a non-empty query
                    break
                if line == "" and not lines:  # ignore leading blank lines
                    continue
                lines.append(line)
            query = "\n".join(lines).strip()
        if not query:
            print("[flow] empty query; exiting.")
            return
