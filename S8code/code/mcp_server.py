...
# ── text measurement tools (Session 8 — Critic-with-tools) ───────────────────
# These exist so the Critic can GROUND a structural verdict in real
# arithmetic instead of eyeballing it. Counting characters, words, lines,
# and syllables is exactly the class of property an LLM rubber-stamps when
# asked to "just check" — see the forcing-query finding in the Session 8
# brief. The model supplies the comparison ("is 7 == 4?"); the tool supplies
# the count. That division of labour is the whole point.

def _count_syllables_word(word: str) -> int:
    """Heuristic English syllable count for a single word. Counts groups of
    consecutive vowels, drops a trailing silent 'e', and never returns < 1
    for a word with letters. Good enough for haiku-style verdicts and, more
    importantly, DETERMINISTIC — the same word always scores the same."""
...
def count_chars(text: str) -> dict:
    """Count characters in text (deterministic). Returns total length including
    spaces, length excluding spaces, and per-line lengths. Use this to verify
    a length constraint like '<= 280 characters'. Example: count_chars("hello world")."""
    lines = text.split("\n")
    return {
        "chars_with_spaces": len(text),
        "chars_without_spaces": len(text.replace(" ", "").replace("\n", "")),
        "line_count": len(lines),
        "per_line_chars": [len(ln) for ln in lines],
    }


@mcp.tool()
def count_words(text: str) -> dict:
    """Count words in text (deterministic, whitespace-split). Returns total word
    count and per-line word counts. Use this to verify a constraint like 'exactly
    7 words' or 'at most 20 words'. Example: count_words("a short line")."""
    lines = text.split("\n")
    return {
        "word_count": len(text.split()),
        "line_count": len(lines),
        "per_line_words": [len(ln.split()) for ln in lines],
    }


@mcp.tool()
def count_syllables(text: str) -> dict:
    """Count syllables per line and in total using a deterministic English
    heuristic (vowel-group counting with silent-e handling). Use this to verify
    a constraint like 'exactly 4-6-4 syllables' or '5-7-5 haiku'. Returns the
    per-line syllable counts in order. Example: count_syllables("an old silent pond")."""
    lines = [ln for ln in text.split("\n")]
    per_line = []
    for ln in lines:
        per_line.append(sum(_count_syllables_word(w) for w in ln.split()))
    # Drop fully-empty leading/trailing lines so a trailing newline does not
    # masquerade as a 0-syllable line and confuse the verdict.
    trimmed = [(ln, s) for ln, s in zip(lines, per_line) if ln.strip()]
    return {
        "per_line_syllables": [s for _, s in trimmed],
        "total_syllables": sum(s for _, s in trimmed),
        "non_empty_line_count": len(trimmed),
        "lines": [ln for ln, _ in trimmed],
    }
