...
# Minimum cosine similarity for a vector hit to count as relevant. FAISS
# IndexFlatIP always returns the k nearest vectors regardless of distance,
# so without this floor a near-empty index returns its entire contents for
# ANY query (the "Say hello returns London population" bug). Cosine over
# L2-normalized retrieval-query/document embeddings; 0.6 is a conservative
# floor — tune if the embedding model changes.
_MIN_VECTOR_SIM = 0.6
...
def _vector_search(
    ...
    *,
    ...
    for item_id, score in hits:
        # Drop neighbours that FAISS ranked but that aren't actually similar.
        # Returning [] here lets read() fall through to the keyword path,
        # whose `score > 0` gate is itself a relevance check.
        if score < _MIN_VECTOR_SIM:
            continue
