...
_TOOL_CATALOG = {
    ...
    # Critic-with-tools measurement primitives. Deterministic counters so a
    # structural verdict (length / word / syllable constraints) is grounded
    # in real arithmetic rather than the model's eyeballing — the Session 8
    # rubber-stamp finding's clean fix.
    "count_chars": {
        "name": "count_chars",
        "description": "Count characters in text (with/without spaces, per line). Deterministic. Use to verify a length constraint.",
        "input_schema": {
            "type": "object",
    ...
    "count_words": {
        "name": "count_words",
        "description": "Count words in text (total and per line). Deterministic. Use to verify a word-count constraint.",
        "input_schema": {
            "type": "object",
            "properties": {"text": {"type": "string"}},
            "required": ["text"],
    ...
    "count_syllables": {
        ...
        "description": "Count syllables per line and total (deterministic English heuristic). Use to verify a syllable constraint like 4-6-4 or 5-7-5.",
        "input_schema": {
            "type": "object",
            "properties": {"text": {"type": "string"}},
            "required": ["text"],
